SELECT
	   ISNULL(product_group._description, '') AS group_product,
	   ...
	   CAST(ISNULL(price.cost, 0)*T1.Fld28225Turnover_ AS NUMERIC(27, 2)) AS Себестоимость,
	   (T1.Fld28226Turnover_ - T1.Fld28228Turnover_) - CAST(ISNULL(price.cost, 0)*T1.Fld28225Turnover_ AS NUMERIC(27, 2)) AS ВаловаяПрибыль,
	   CASE
		   WHEN CAST(ISNULL(price.cost, 0)*T1.Fld28225Turnover_ AS NUMERIC(27, 2)) = 0.0
			   THEN 0
		   ELSE CAST(100*((T1.Fld28226Turnover_ - T1.Fld28228Turnover_) - CAST(ISNULL(price.cost, 0)*T1.Fld28225Turnover_ AS NUMERIC(27, 2)))/CAST(ISNULL(price.cost, 0)*T1.Fld28225Turnover_ AS NUMERIC(27, 2)) AS NUMERIC(27, 2))
	   END AS Рентабельность,
	   T1.period_ AS period_month
  FROM (SELECT
		...
		HAVING (CAST(SUM(T2._Fld28226) AS NUMERIC(27, 2))) <> 0.0
			...
			OR (CAST(SUM(T2._Fld28228) AS NUMERIC(27, 2))) <> 0.0) AS T1
	   LEFT JOIN _Reference211 AS Справочник_Номенклатура
		   LEFT JOIN _Reference212 AS product_group
		   ON  Справочник_Номенклатура._Fld3307RRef = product_group._IDRRef
	   ON  T1.Fld28216RRef = Справочник_Номенклатура._IDRRef
	   LEFT JOIN (SELECT
						 T1.Fld26099RRef,
					     T1.Fld26102_ AS cost
					FROM (SELECT
							...
							FROM (SELECT
							   ...
							   WHERE T3._Active = 0x01
								 AND ((T3._Fld26098RRef = 0x80630025904F2CAA11E2B0AAFB9CDDC8))
							   GROUP BY T3._Fld26098RRef,
									 T3._Fld26099RRef,
									 T3._Fld26100RRef) AS T2
								 ...
								 ON T2.Fld26098RRef = T4._Fld26098RRef
									...
									AND T2.MAXPERIOD_ = T4._Period) AS T1) AS price
	   ON  T1.Fld28216RRef = price.Fld26099RRef
